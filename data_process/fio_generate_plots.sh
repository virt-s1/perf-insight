...
# Description:
#   This script uses the files generated by FIO to create nice graphs.
...
if [ ! -x "$GNUPLOT" ]; then
    echo "ERROR: You need gnuplot installed to generate graphs"
    exit 1
fi
...
plot() {
    ...
    if [ $i -eq 0 ]; then
        echo -e "ERROR: No log files found! Pattern: ${TITLE}_${FILETYPE}.[0-9]*.log\n"
        return
    fi
